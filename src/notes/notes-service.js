const NotesService = {
	getNotesByDogId(db, dogId) {
		return db.from("notes").select("*").where("dog_id", dogId);
	},
	...
	deleteByNoteId(db, id) {
		return db.from("notes").where({ id }).delete();
	},
	deleteNoteByDogId(db, dogId) {
		return db.from("notes").where("dog_id", dogId).delete();
	},
};
